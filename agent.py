...
def invoke_agent(runner: Runner, prompt: str, user_id: str, session_id: str) -> str:
    """
    ...
    for attempt in range(1, 4):
        try:
            ...
            # Extract the final text from the model's turn event
            response_text = ""
            for event in events:
            ...
            # Fallback to output or transcription if content is empty
    ...
    print(f"[Orchestrator Fallback] All 3 agent invocation retries failed. Activating local deterministic response...")
...
class SilverGroveOrchestrator:
    ...
    using stateful ADK Runner sessions.
    """
    
    def run_health_check(self, resident_id: str) -> dict:
        ...
        print("\n[Orchestrator] Activating Sensory Guardian...")
        sensory_prompt = (
            f"Analyze vital sign readings, baselines, and 5-day gait history for resident '{resident_id}'. "
            "Report if there are any significant anomalies (elevated blood pressure or gait speed slowdown >=15%)."
        )
        try:
            sensory_response = invoke_agent(sensory_runner, sensory_prompt, user_id, run_session_id)
            trajectory["sensory_guardian"] = {
                "status": "COMPLETED",
                "output": sensory_response
            }
            print(f"[Sensory Guardian Output]:\n{sensory_response}\n")
        except Exception as e:
            print(f"ERROR executing Sensory Guardian Agent: {e}")
        ...
        has_anomaly = any(
            ...
            for word in ["ANOMALY", "WARNING", "DEGRADATION", "BP ELEVATED", "SLOWDOWN", "ELEVATED", "SPIKE", "ALERT"]
        )
        
        if not has_anomaly:
            trajectory["summary"] = "Vitals are normal. No escalation needed."
            return trajectory
            
        # --- Step 2: Medical Compliance Clinical Correlation ---
        print("[Orchestrator] Anomaly detected! Activating Medical Compliance Agent...")
        compliance_prompt = (
        ...
        try:
            compliance_response = invoke_agent(compliance_runner, compliance_prompt, user_id, run_session_id)
            trajectory["medical_compliance"] = {
                "status": "COMPLETED",
                "output": compliance_response
            }
            print(f"[Medical Compliance Output]:\n{compliance_response}\n")
        except Exception as e:
            print(f"ERROR executing Medical Compliance Agent: {e}")
        ...
        print("[Orchestrator] Activating Cognitive Companion for active resident care...")
        companion_prompt = (
        ...
        try:
            companion_response = invoke_agent(companion_runner, companion_prompt, user_id, run_session_id)
            trajectory["cognitive_companion"] = {
                "status": "COMPLETED",
                "output": companion_response
            }
            print(f"[Cognitive Companion Output]:\n{companion_response}\n")
        except Exception as e:
            print(f"ERROR executing Cognitive Companion Agent: {e}")
            trajectory["cognitive_companion"] = {"status": "FAILED", "output": str(e)}
            return trajectory

        # --- Step 4: Care Coordinator A2A Gateway Alert Routing ---
        print("[Orchestrator] Activating Care Coordinator for external A2A alerting...")
        coordinator_prompt = (
        ...
        try:
            coordinator_response = invoke_agent(coordinator_runner, coordinator_prompt, user_id, run_session_id)
            trajectory["care_coordinator"] = {
                "status": "COMPLETED",
                "output": coordinator_response
            }
            print(f"[Care Coordinator Output]:\n{coordinator_response}\n")
        except Exception as e:
            print(f"ERROR executing Care Coordinator Agent: {e}")
...
if __name__ == "__main__":
    ...
    res = orchestrator.run_health_check("martha_001")
