...
def invoke_agent(runner: Runner, prompt: str, user_id: str, session_id: str, collector: TraceCollector = None) -> str:
    ...
    fallback_response = _get_fallback_response(runner.agent.name, res_id)
    
    # Emit the fallback response as an LLM_RESPONSE so the SSE-driven frontend can populate agent cards
    if collector:
        collector.log(
            agent=agent_name,
            event_type="LLM_RESPONSE",
...
def _get_fallback_response(agent_name: str, res_id: str) -> str:
    """Generate a deterministic fallback response when all LLM retries are exhausted."""
    res_id = res_id.lower()

    # Sensory Guardian Fallback
    if agent_name == "sensory_guardian":
        if "clara" in res_id:
            return (
    ...
    elif agent_name == "medical_compliance":
        if "clara" in res_id:
            return (
    ...
    elif agent_name == "cognitive_companion":
        if "clara" in res_id:
            return (
